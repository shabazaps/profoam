...
class Test_Checkout(BaseClass):
    ...
    def test_checkout(self, setup):
        ...
        checkout.diff_address()
        logger.info("Different Address button clicked successfully")

        # Different address form is visible

        # Enter first name
        checkout.ship_fname_input()

        # Enter last name
        checkout.ship_lname_input()

        # Select United States from dropdown
        checkout.ship_country()

        # Select Maryland from dropdown
        checkout.ship_province()

        # Enter shipp_address 1
        checkout.ship_add_1()

        # Enter shipp_address 2
        checkout.ship_add_2()

        # Enter Ship City
        checkout.ship_city_input()

        # Enter Ship Zip Code
        checkout.ship_zip_input()
        logger.info("Different shipping address entered successfully")
